...
<body>
  ...
  <form id="form1" name="form1" class="form" method="get" action="Fournisseur élec2.php">
    <div class="container">
          <label for="textfield"><b>la clé (code Fe):</b></label>
          <input type="text" name="X" id="textfield" placeholder="la clé (code Fe)..." >
      

          <label for="textfield2"><b>Nom du fournisseur : </b></label>
          <input type="text" name="A" placeholder="Nom du fournisseur..." >
...
<?php
echo '<p>les clés (codeFe) stockés actuélment sont: 3 4</p>';
...
$r="select `$t`.`codeFe` from `$t` where `$t`.`codeFe`='$X'";
$req=$id->prepare($r);
$e=$req->execute();
...
while($enreg){
	...
	echo " clé valide";
		 $r="update `$t` set `$t`.`nom`='$A' where `$t`.`codeFe`='$X' ";
		$req=$id->prepare($r);
        $e=$req->execute();
