...
<body>
  ...
  <form id="form1" name="form1" class="form" method="get" action="Articles informatiques2.php">
    <div class="container">
          <label for="textfield"><b>La clé</b></label>
  ...
  <?php
  echo '<p>les clés (codeA) stockés actuélment sont: 77x 88x 99x</p>';
  ...
  $r="select `$t`.`codeA` from `$t` where `$t`.`codeA`='$X'";
  $req=$id->prepare($r);
  $e=$req->execute();
  ...
  while($enreg){
    ...
    echo " clé valide";
      $r="update `$t` set `$t`.`type`='$B',`$t`.`nbr d'unités`='$C' where `$t`.`codeA`='$X' ";
      $req=$id->prepare($r);
          $e=$req->execute();
