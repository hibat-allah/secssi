...
<body>
  ...
  <form id="form1" name="form1" class="form" method="get" action="Fournisseur info2.php">
    <div class="container">
          <label for="textfield"><b>la clé (code Fi):</b></label>
          <input type="text" name="X" id="textfield" placeholder="la clé (code Fi)..." >
...
<?php
echo '<p>les clés (codeFi) stockés actuélment sont: 1 2</p>';
...
$r="select `$t`.`codeFi` from `$t` where `$t`.`codeFi`='$X'";
$req=$id->prepare($r);
$e=$req->execute();
...
while($enreg){
	...
	echo " clé valide";
		 $r="update `$t` set `$t`.`nom`='$A' where `$t`.`codeFi`='$X' ";
		$req=$id->prepare($r);
        $e=$req->execute();
